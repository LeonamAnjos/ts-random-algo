...
describe("mostRequestedResource()", () => {
  describe("WHEN time period is default", () => {
    [
      {
        input: [],
  ...
  describe("WHEN time period is 1s", () => {
    [
      {
        input: [
          ["300", "user_1", "resource_3"],
        ...
        expected: { resource: "resource_3", count: 7 },
      },
      {
        input: [
          ["58523", "user_1", "resource_1"],
        ...
        expected: { resource: "resource_3", count: 5 },
      },
    ].forEach(({ input, expected }, index) => {
      it(`#${index}`, () => {
        expect(mostRequestedResource(input, 1000)).toEqual(expected);
      });
    });
  });
});
